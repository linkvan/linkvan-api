...
RSpec.describe Locations::GoogleMaps::EmbedMapService, type: :service do
  ...
  describe "initialization" do
    ...
    it "creates a URI object with the correct base URL" do
      ...
      expect(service.uri.to_s).to start_with("https://maps.googleapis.com/maps/embed/v1/place")
    end
  ...
  describe "#call" do
    ...
    it "has the correct path" do
      expect(result.path).to eq("/maps/embed/v1/place")
    end
    ...
    describe "query parameters" do
      ...
      it "includes q parameter with coordinates (instead of markers)" do
        expect(query_params["q"]).to eq("49.243463,-123.106431")
      end

      it "includes key parameter with GOOGLE_KEY from environment" do
        expect(query_params["key"]).to eq(described_class::GOOGLE_KEY)
      end

      it "does not include size parameter (commented out in service)" do
        expect(query_params).not_to have_key("size")
      end

      it "does not include markers parameter (commented out in service)" do
        expect(query_params).not_to have_key("markers")
      end

      it "does not include signature parameter when GOOGLE_SIGNATURE is nil" do
        expect(query_params).not_to have_key("signature")
      end

      context "when GOOGLE_SIGNATURE is present" do
        before do
          stub_const("Locations::GoogleMaps::EmbedMapService::GOOGLE_SIGNATURE", "test_signature")
        end
    ...
    describe "coordinate rounding behavior" do
      context "with many decimal places" do
        ...
        it "rounds to 6 decimal places in q parameter" do
          result = high_precision_service.call
          query_params = URI.decode_www_form(result.query).to_h
          expect(query_params["q"]).to eq("49.243463,-123.106431")
        end
      ...
      context "with coordinates that need rounding up" do
        ...
        it "rounds correctly" do
          ...
          expect(query_params["q"]).to eq("49.243464,-123.106432")
        end
      end
      ...
      context "with zero coordinates" do
        ...
        it "handles zero coordinates correctly" do
          ...
          expect(query_params["q"]).to eq("0,0")
        end
      end
    end

    describe "query parameter behavior" do
      it "uses q parameter instead of markers" do
        result = service.call
        query_params = URI.decode_www_form(result.query).to_h
        expect(query_params).to have_key("q")
        expect(query_params).not_to have_key("markers")
      end

      it "q parameter contains coordinates" do
        result = service.call
        query_params = URI.decode_www_form(result.query).to_h
        expect(query_params["q"]).to eq("49.243463,-123.106431")
      end

      it "center and q parameters use the same coordinates" do
        result = service.call
        query_params = URI.decode_www_form(result.query).to_h
        expect(query_params["center"]).to eq(query_params["q"])
      end
    end
    ...
    describe "edge cases" do
      ...
      context "with very large coordinates" do
        ...
        it "handles large coordinates" do
          ...
          expect(query_params["q"]).to eq("999.999999,-999.999999")
        end
      end
      ...
      context "with very small coordinates" do
        ...
        it "handles very small coordinates" do
          ...
          expect(query_params["q"]).to eq("0.0,-0.0")
        end
      end
    end
    ...
    describe "URL encoding" do
      it "properly encodes query parameters" do
        ...
        expect(result.query).to include("q=49.243463%2C-123.106431")
      end
  ...
  describe "environment variable handling" do
    it "has the stubbed GOOGLE_KEY" do
      expect(described_class::GOOGLE_KEY).to eq("test_google_key")
    end

    it "includes the stubbed token in query parameters" do
      result = service.call
      query_params = URI.decode_www_form(result.query).to_h
  ...
  describe "private methods" do
    ...
    describe "#markers" do
      it "returns an array with marker components (though not used in embed API)" do
    ...
    describe "#query_params" do
      ...
      it "does not include commented out parameters" do
        expect(query_params).not_to have_key(:size)
        expect(query_params).not_to have_key(:markers)
      end
      ...
      it "uses coordinates for q parameter" do
        expect(query_params[:q]).to eq("49.243463,-123.106431")
      end

      it "center and q parameters are identical" do
        expect(query_params[:center]).to eq(query_params[:q])
      end
    end
  end
  ...
  describe "class method interface" do
    ...
    it "class method handles multiple arguments correctly" do
      ...
      expect(query_params["q"]).to eq("40.7128,-74.006")
    end
  end
  ...
  describe "configuration independence" do
    it "uses the actual MAP_CONFIG values" do
      ...
      expect(query_params["zoom"]).to eq("14")
      expect(query_params["maptype"]).to eq("roadmap")
      # Shouldn't have size since it's commented out
      expect(query_params).not_to have_key("size")
    end
  end

  describe "comparison with StaticMapService" do
    let(:static_service) { Locations::GoogleMaps::StaticMapService.new(latitude, longitude) }

    it "uses different base URL" do
      static_result = static_service.call
      embed_result = service.call

      expect(static_result.path).to eq("/maps/api/staticmap")
      expect(embed_result.path).to eq("/maps/embed/v1/place")
    end

    it "uses different query parameters structure" do
      static_result = static_service.call
      embed_result = service.call

      static_params = URI.decode_www_form(static_result.query).to_h
      embed_params = URI.decode_www_form(embed_result.query).to_h

      expect(static_params).to have_key("markers")
      expect(static_params).not_to have_key("q")

      expect(embed_params).to have_key("q")
      expect(embed_params).not_to have_key("markers")
    end

    it "both use the same coordinate rounding" do
      static_result = static_service.call
      embed_result = service.call
      ...
      expect(static_params["center"]).to eq(embed_params["center"])
    end
  end

  describe "real-world usage scenarios" do
    it "handles Vancouver coordinates" do
      vancouver_service = described_class.new(49.2827, -123.1207)
      result = vancouver_service.call
      query_params = URI.decode_www_form(result.query).to_h

      expect(query_params["center"]).to eq("49.2827,-123.1207")
      expect(query_params["q"]).to eq("49.2827,-123.1207")
    end

    it "handles New York coordinates" do
      ny_service = described_class.new(40.7128, -74.0060)
      result = ny_service.call
      query_params = URI.decode_www_form(result.query).to_h
    ...
    it "handles London coordinates" do
      london_service = described_class.new(51.5074, -0.1278)
      result = london_service.call
      query_params = URI.decode_www_form(result.query).to_h

      expect(query_params["center"]).to eq("51.5074,-0.1278")
      expect(query_params["q"]).to eq("51.5074,-0.1278")
    end
  end
end
